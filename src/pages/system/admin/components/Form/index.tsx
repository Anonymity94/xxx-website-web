...
import { passwordMsg, passwordReg, usernameMsg, usernameReg } from '@/utils/utils';
...
const AdminForm = ({ submittingLoading, detail = {} as IAdmin, dispatch }: IAdminFormProps) => {
  const isUpdate = useMemo(() => {
    return !!detail.id;
  }, [detail?.id]);

  const goBack = () => {
    history.push('/system/user/admin/list');
  };
  ...
  const handleFinish = (values: Record<string, any>) => {
    ...
    Modal.confirm({
      ...
      onOk: () => {
        dispatch({
          type: 'adminModel/createOrUpdateAdmin',
          payload: postData,
...
export default connect(({ loading }: ConnectState) => ({
  submittingLoading: loading.effects['adminModel/createOrUpdateAdmin'],
}))(AdminForm);
