...
interface ILegalAffairsFormProps {
  dispatch: Dispatch;
  detail?: ILegalAffairs;
  submittingLoading?: boolean;
}
const LegalAffairsForm = ({
  submittingLoading,
  detail = {} as ILegalAffairs,
  dispatch,
}: ILegalAffairsFormProps) => {
  const isUpdate = useMemo(() => {
    return !!detail.id;
  }, [detail?.id]);

  const goBack = () => {
    history.push('/system/user/legal-affairs/list');
  };
  ...
  const handleFinish = (values: Record<string, any>) => {
    ...
    Modal.confirm({
      ...
      onOk: () => {
        dispatch({
          type: 'legalAffairsModel/createOrUpdateLegalAffairs',
          payload: postData,
...
export default connect(({ loading }: ConnectState) => ({
  submittingLoading: loading.effects['legalAffairsModel/createOrUpdateLegalAffairs'],
}))(LegalAffairsForm);
