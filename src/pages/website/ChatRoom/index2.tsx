...
import { queryChatContactList } from '@/services/chat';
...
const ChatRoom = () => {
  ...
  // 如果是律师、法务，支持展示聊天记录
  useEffect(() => {
    if (pageReady && (access.isLawyer || access.isLegalAffairs)) {
      queryChatContactList({
        ...
        if (success) {
          ...
          const newMessageMap: IMessageMap = {};
          for (let i = 0; i < result.length; i += 1) {
            const row = result[i];
            let newMessageList: IMessage[] = [];
            try {
              newMessageList = JSON.parse(`[${row.record}]`);
            } catch (error) {
              newMessageList = [];
            }

            // 过滤掉 null 的消息
            newMessageList = newMessageList.filter((el) => el);

            row.recordList = newMessageList;
            newContactList.push(row);

            // 更新消息记录
            newMessageMap[`${row.senderId}_${row.senderRole}`] = newMessageList;
          }
          ...
          if (newContactList.length > 0) {
            setReceiverUser({
              receiverId: newContactList[0].senderId,
              receiverRole: newContactList[0].senderRole,
              receiverName: newContactList[0].senderName,
            });
          }
        }
      });
    }
  }, [pageReady, access, currentUser, dateRange]);
  ...
  useEffect(() => {
    ...
    // 30分钟重连
    socketRef.current.reconnectInterval = 30 * 1000;
    socketRef.current.timeoutInterval = 38 * 1000;
  ...
  const updateMessageMap = useCallback(
    (message: IMessage) => {
      const key = message.isMy
        ? `${message.receiverId}_${message.receiverRole}`
  ...
  useEffect(() => {
    ...
    socketRef.current.onmessage = (e) => {
      ...
      if (
        ...
        antdMessage.warning('人工坐席繁忙，请稍后再试。');
        return;
      }
  ...
  }, [socketRef, currentUser, receiverUser, currentOrderId, pageReady, updateMessageMap]);
  ...
  const handleSendMessage = (message?: IMessage) => {
    // 检查socket状态，开启的时候才可以
    if (!socketIsOpen()) {
      // antdMessage.warning('对话准备中...，请稍后再试');
      return;
    }
      ...
      <div className={styles['chat-room-wrap']}>
        {showContract && (
          <div className={styles['chat-room-left']}>
            <div className={styles['chat-room-left-header']}>最近联系人</div>
                ...
                {contactList.map((contact) => (
                  <li
                    ...
                    className={classNames({
                      [styles.active]: contact.senderId === receiverUser.receiverId,
                    })}
                    onClick={() =>
                      setReceiverUser({
                        receiverId: contact.senderId,
                        receiverRole: contact.senderRole,
                        receiverName: contact.senderName,
                      })
                    }
                  >
                    {contact.senderName}
                    {`${contact.senderId}_${contact.senderRole}` !==
                      `${receiverUser.receiverId}_${receiverUser.receiverRole}` && (
                      <Badge
                        count={unReadMap[`${contact.senderId}_${contact.senderRole}`] || 0}
